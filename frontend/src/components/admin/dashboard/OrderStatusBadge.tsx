...
const statusUiMap: Record<string, {label: string; className: string}> = {
    ...
    PROCESSING: {label: "Đang xử lý", className: "bg-violet-50 text-violet-700 ring-violet-100"},
    SHIPPING: {label: "Đang giao", className: "bg-cyan-50 text-cyan-700 ring-cyan-100"},
    DELIVERED: {label: "Đã giao", className: "bg-emerald-50 text-emerald-700 ring-emerald-100"},
...
export default function OrderStatusBadge({status}: OrderStatusBadgeProps) {
    ...
    return (
        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${ui.className}`}>
            {ui.label}
        </span>
    )
}
