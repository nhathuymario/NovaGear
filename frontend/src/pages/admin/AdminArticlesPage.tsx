...
export default function AdminArticlesPage() {
    ...
    const handleToggleStatus = async (a: ArticleItem) => {
        ...
        try {
            await updateArticle(a.id, {status: newStatus});
            fetchArticles()
        } catch { /* silently handle */
        }
    }
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                    <button onClick={openGenerate}
                            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/25 transition hover:shadow-xl">
                            ...
                            <tr key={a.id} className="transition hover:bg-slate-50/50">
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-50">
                                ...
                                <td className="hidden px-4 py-3 md:table-cell">
                                    <span
                                        className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600">{a.category}</span>
                                </td>
                                <td className="px-4 py-3">
                                    <button onClick={() => handleToggleStatus(a)}
                                            className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${a.status === "PUBLISHED" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                                        {a.status === "PUBLISHED" ? "Đã xuất bản" : "Bản nháp"}
                                    </button>
                                </td>
                                <td className="hidden px-4 py-3 text-xs text-slate-500 md:table-cell">
                                    <span className="flex items-center gap-1"><Calendar
                                        className="h-3 w-3"/>{formatDate(a.created_at)}</span>
                                </td>
                                <td className="px-4 py-3 text-right">
                                    <div className="flex items-center justify-end gap-1">
                                        <a href={`/tech/${a.slug}`} target="_blank" rel="noopener noreferrer"
                                        ...
                                        {deleteConfirm === a.id ? (
                                            <div className="flex items-center gap-1">
                                                <button onClick={() => handleDelete(a.id)}
                                                        className="rounded-lg bg-red-500 p-2 text-white"><Check
                                                    className="h-4 w-4"/></button>
                                                <button onClick={() => setDeleteConfirm(null)}
                                                        className="rounded-lg bg-slate-200 p-2 text-slate-600"><X
                                                    className="h-4 w-4"/></button>
                                            </div>
                                        ) : (
                ...
                <div className="flex justify-center gap-1">
                    {Array.from({length: totalPages}).map((_, i) => (
                        <button key={i} onClick={() => setPage(i)}
                                className={`h-8 w-8 rounded-lg text-xs font-semibold ${i === page ? "bg-indigo-600 text-white" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>{i + 1}</button>
                    ))}
                </div>
            )}
                ...
                <div
                    ...
                    <motion.div initial={{opacity: 0, scale: 0.95}} animate={{opacity: 1, scale: 1}}
                        ...
                        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                            ...
                            <button onClick={closeModal} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X
                        ...
                        <div className="max-h-[70vh] overflow-y-auto p-6">
                            {modalMode === "generate" ? (
                                <div className="space-y-4">
                                    <div>
                                        <label className="mb-1 block text-sm font-semibold text-slate-700">Chủ đề
                                            *</label>
                                        <input value={aiTopic} onChange={e => setAiTopic(e.target.value)}
                                               placeholder="VD: RTX 5090 vs RTX 4090 — So sánh hiệu năng"
                                    ...
                                    <div>
                                        <label className="mb-1 block text-sm font-semibold text-slate-700">Từ khóa (cách
                                            nhau bằng dấu phẩy)</label>
                                        <input value={aiKeywords} onChange={e => setAiKeywords(e.target.value)}
                                               placeholder="GPU, gaming, benchmark"
                                               className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"/>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="mb-1 block text-sm font-semibold text-slate-700">Phong
                                                cách</label>
                                            <div className="relative">
                                                <select value={aiTone}
                                                        onChange={e => setAiTone(e.target.value as typeof aiTone)}
                                                        className="w-full appearance-none rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-indigo-400">
                                                    {TONE_OPTIONS.map(t => <option key={t.value}
                                        ...
                                        <div>
                                            <label className="mb-1 block text-sm font-semibold text-slate-700">Danh
                                                mục</label>
                                            <div className="relative">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="mb-1 block text-sm font-semibold text-slate-700">Tiêu đề
                                            *</label>
                                        <input value={title} onChange={e => setTitle(e.target.value)}
                                               className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"/>
                                    </div>
                                    <div>
                                        <label className="mb-1 block text-sm font-semibold text-slate-700">Ảnh bìa (URL
                                            hoặc Tải lên)</label>
                                        <div className="flex items-center gap-3">
                                            {coverImageUrl && (
                                                <div
                                                    className="h-10 w-16 shrink-0 overflow-hidden rounded bg-slate-100">
                                                    <img
                                                        src={coverImageUrl.startsWith('http') ? coverImageUrl : (coverImageUrl.startsWith('/api') ? coverImageUrl : `/api${coverImageUrl.startsWith('/') ? '' : '/'}${coverImageUrl}`)}
                                                        alt="Cover" className="h-full w-full object-cover"/>
                                                </div>
                                            )}
                                            <input value={coverImageUrl}
                                                   onChange={e => setCoverImageUrl(e.target.value)}
                                                   placeholder="https://..."
                                                   className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"/>
                                            <label
                                                className="shrink-0 cursor-pointer rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-200">
                                                Tải lên
                                    ...
                                    <div>
                                        <label className="mb-1 block text-sm font-semibold text-slate-700">Tóm
                                            tắt</label>
                                        <textarea value={summary} onChange={e => setSummary(e.target.value)} rows={2}
                                                  className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"/>
                                    </div>
                                    <div>
                                        <label className="mb-1 block text-sm font-semibold text-slate-700">Nội dung
                                            (Markdown) *</label>
                                        <textarea value={content} onChange={e => setContent(e.target.value)} rows={12}
                                                  className="w-full rounded-xl border border-slate-200 px-4 py-2.5 font-mono text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"/>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label className="mb-1 block text-sm font-semibold text-slate-700">Trạng
                                                thái</label>
                                            <div className="relative">
                                                <select value={status}
                                    ...
                                    <div>
                                        <label className="mb-1 block text-sm font-semibold text-slate-700">Tags (cách
                                            nhau bằng dấu phẩy)</label>
                                        <input value={tagsStr} onChange={e => setTagsStr(e.target.value)}
                                               placeholder="laptop, gaming, review"
                                               className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"/>
                                    </div>

                                    <div>
                                        <label className="mb-2 block text-sm font-semibold text-slate-700">Hình ảnh bài
                                            viết</label>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                                            {images.map((img, idx) => (
                                                <div key={idx}
                                                     className="relative group overflow-hidden rounded-xl border border-slate-200 bg-slate-50 aspect-video">
                                                    <img
                                                        src={img.imageUrl.startsWith('http') ? img.imageUrl : (img.imageUrl.startsWith('/api') ? img.imageUrl : `/api${img.imageUrl.startsWith('/') ? '' : '/'}${img.imageUrl}`)}
                                                    ...
                                                    <div
                                                        className="absolute bottom-0 left-0 right-0 bg-black/50 p-2 opacity-0 transition-opacity group-hover:opacity-100">
                                                        <input
                                                            value={img.caption || ""}
                                                            onChange={e => {
                                                                const newImages = [...images];
                                            ...
                                            <label
                                                className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 aspect-video transition hover:border-indigo-400 hover:bg-indigo-50">
                                                ...
                                                {uploading ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <span
                                                            className="text-xs font-medium text-slate-500">Tải ảnh lên</span>
                                                    </>
                                                )}
                                            </label>
                                        </div>
                        ...
                        <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
                            <button onClick={closeModal}
                                    className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50">Hủy
                            </button>
                            <button
